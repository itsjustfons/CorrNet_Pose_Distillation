...
from slr_network_2head_v4 import SLRModel
import torch
...
model_weights = '/data/group1/z40575r/CorrNet_pose_distillation/CorrNet/work_dir/keypoint_regression_img_grad_accum_v13_max16_bs8_a1_b1/epoch2_step60000.pt'  #TODO: replace with your path
...
video = '/data/group1/z40575r/GloFE/cropped-vid/XySXQIrwypg-00-00-29.533-00-00-34.966.mp4'
#instead of glob, use CV
...
frame_count = 0
frame_interval = 1
offset = int(torch.randint(0, frame_interval, [1])) #do we need this
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break
...
if 'phoenix' in dataset:
    ...
    model.conv2d.layer4.conv_back.register_forward_hook(forward_hook)	
else:
    model.conv2d.corr3.conv_back.register_forward_hook(forward_hook)  # For CSL-Daily
'''

def backward_hook(module, grad_input, grad_output):
    grad_block.append(grad_output[0])

target_layer = model.conv2d.layer4[-1]
