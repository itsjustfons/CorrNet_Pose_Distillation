...
import modules.resnet_stripped_full as resnet
import os
...
class SLRModel(nn.Module):
    def __init__(
        ...
        self.conf_head = nn.Linear(512, 133)  # one confidence per joint
    ...
    def forward(self, x, len_x=None, label=None, label_lgt=None):
        if len(x.shape) == 5:
            #breakpoint()
            B, T, C, H, W = x.shape
            framewise = self.conv2d(x.permute(0,2,1,3,4).contiguous()).view(B, T, -1)#.permute(0,2,1) # btc -> bct. batch time, 512, 7, 7
            #B, T, C, H, W = framewise.shape
            #breakpoint()
            framewise = framewise.view(B * T, -1) 
