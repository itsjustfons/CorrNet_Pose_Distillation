...
class SLRModel(nn.Module):
    ...
    def forward(self, x, len_x=None, label=None, label_lgt=None):
        ...
        #breakpoint()
        #pdb.set_trace()
        #self.save_individual_heatmaps(output)
        #self.save_aggregated_heatmaps_per_sample(output) #to be used for visualization
        return {
    ...
    def keypoint_loss(self, ret_dict, label, target_weight, alpha = 1.0, beta = 1.0):
        #target_weight = target_weight.expand(-1, -1, -1, 2)
        B, T, J, C = ret_dict.shape 
        #print("output shape", ret_dict.shape)
