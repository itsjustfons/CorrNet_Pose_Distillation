...
def load_corrnet():
    print("Loading model")
    model_class = import_class('slr_network_sequential_v3.SLRModel')
    model = model_class(c2d_type='resnet34', num_classes=1296, conv_type=2, )

    #shutil.copy2(inspect.getfile(model_class), self.arg.work_dir)

    model = load_model_weights(model, '/data/group1/z40575r/CorrNet_pose_distillation/CorrNet/work_dir/keypoint_regression_seq_v4/epoch2_step40000.pt')
    #move model to GPU
...
def main():
    ...
    arg_dict = {
        ...
        'output_root': 'keypoints_regression_seq_v4_40k_ep2',
        'args': args,
    }
    ...
    #process_single_video("/data/group1/z40575r/TimeSformer_GloFE/TimeSformer/datasets/how2sign/256/_-adcxjm1R4_0-8-rgb_front.mp4", arg_dict)
    


def process_single_video(video_path, arg_dict, corrnet_model, max_frames=256):
    #start = time.time()
    ...
    cap.release()
    #video data is now being held in opencv_frames. Convert into tensor video_tensor
    video_tensor = torch.from_numpy(np.stack(opencv_frames, axis=0)).float().permute(0,3,1,2)
    #video_tensor = video_tensor / 127.5 - 1.0
    video_tensor = video_tensor.unsqueeze(0).to('cuda')
    #breakpoint()
    total_frames = len(opencv_frames)
    chunks = []
    sample_id = video_path.split('/')[-1][:-4]
    video_tensor = video_tensor / 127.5 - 1.0 #normalize

    for start in range(0, total_frames, max_frames):
        end = min(start + max_frames, total_frames)
        batched_tensor = video_tensor[0][start:end] 
        batched_tensor = batched_tensor.unsqueeze(0)
        #breakpoint()
        keypoint_sequence = corrnet_model(batched_tensor)

        #Append chunks
        chunks.append(keypoint_sequence['predict_heatmap'][0].detach().cpu().numpy())
        del batched_tensor, keypoint_sequence
