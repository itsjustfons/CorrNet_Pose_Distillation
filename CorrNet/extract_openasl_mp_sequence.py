...
def load_corrnet():
    ...
    model = load_model_weights(model, '/data/group1/z40575r/CorrNet_pose_distillation/CorrNet/work_dir/keypoint_regression_img_grad_accum_v5/epoch0_step130000.pt')
    #move model to GPU
...
def main():
   ...
   # if dataset_info is None:
    ...
    arg_dict = {
        ...
        'output_root': 'keypoints_regression_seqential',
        'args': args,
    }
...
def process_single_video(video_path, arg_dict, corrnet_model, max_frames=500):
    ...
    video_array_rgb = np.stack([cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in video])

    sample_id = video_path.split('/')[-1][:-4]

    # Convert to torch tensor
    video_array_rgb = torch.from_numpy(video_array_rgb).float()
    total_frames = video_array_rgb.shape[0]
    print(video_array_rgb.shape)
    # Split into chunks if needed
    chunks = []
    for start in range(0, total_frames, max_frames):
        end = min(start + max_frames, total_frames)
        batched_tensor = video_array_rgb[start:end].unsqueeze(0).to('cuda')  # Shape: (1, T, H, W, 3)
        #breakpoint()
        keypoint_sequence = corrnet_model(batched_tensor.permute(0, 1, 4, 2, 3))
    ...
    for frame_id, cur_frame in enumerate(mmcv.track_iter_progress(video)):
        ...
        with torch.no_grad():
            ...
            #transform heatmap to keypoints
            #breakpoint()
