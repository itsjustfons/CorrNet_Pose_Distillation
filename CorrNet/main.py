    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                # train end2end model
                alpha = self.arg.loss_weights['alpha']
                beta = self.arg.loss_weights['beta']
                conf_loss_mode = self.arg.loss_weights['conf_loss_mode']
                seq_train(self.data_loader['train'], self.model, self.optimizer,
                          self.device, epoch, self.recoder, self.arg.work_dir, alpha, beta, conf_loss_mode)
                eval_model = False
                print(eval_model)
                if eval_model:
                    dev_pck = seq_eval(self.arg, self.data_loader['valid'], self.model, self.device,
                                       'valid', epoch, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
                    self.recoder.print_log("Dev PCK: {:05.2f}%".format(dev_pck))
                '''
                if best_dev > dev_pck: #We want a lower loss value
                    best_dev = dev_pck
                    best_epoch = epoch
                ...
                if save_model:
                    #model_path = "{}dev_{:05.2f}_epoch{}_model.pt".format(self.arg.work_dir, dev_pck, epoch)
            ...
            dev_pck = seq_eval(self.arg, self.data_loader["valid"], self.model, self.device,
                               "valid", 6667, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
