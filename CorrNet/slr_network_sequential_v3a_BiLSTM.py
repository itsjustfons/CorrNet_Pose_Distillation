...
import modules.resnet_seq_big as resnet
import os
...
class SLRModel(nn.Module):
    def __init__(
        ...
        self.temporal_model = BiLSTMLayer(rnn_type='LSTM', input_size=25088, hidden_size=25088,
    ...
    def forward(self, x, len_x=None, label=None, label_lgt=None):
        if len(x.shape) == 5:
            ...
            x = conv1d_outputs['visual_feat'].permute(1,0,2)
            x = x.permute(1,0,2)
            lgt = conv1d_outputs['feat_len'] #fix RuntimeError: Expected `len(lengths)` to be equal to batch_size, but got 2 (batch_size=130)
            if lgt is None:
                lgt = [temp]
            '''
            framewise = framewise.permute(2,0,1)
            #breakpoint()
            if len_x is None:
                len_x = temp
            if not isinstance(len_x, torch.Tensor):
                len_x = torch.tensor([len_x], dtype=torch.long)
            breakpoint()
            tm_outputs = self.temporal_model(framewise, len_x)['predictions']
            tm_outputs = tm_outputs.permute(1,0,2)
            #breakpoint()
            #framewise = framewise.view(B,T, C * H * W)

            #get xy coordinates
            output_xy = self.temporal_keypoint_head(tm_outputs)
            T = tm_outputs.size(1)
            output_xy = output_xy.view(batch, T, 133, 2) 
            output_xy = torch.relu(output_xy)

            #get confidence values
            output_conf = self.conf_head(tm_outputs).unsqueeze(-1)
