'''
This version of the data loader feeds a non-sequential input. It feeds more video frames from each video instead of just the first frame
'''
import os
import cv2
import sys
...
class BaseFeeder(data.Dataset):
    ...
    def read_video(self, index):
        ...
        #if frame_list and pose_output are not equal, crop the longer to match the shorter
        #eg if pose data is 8 frames and video is 10 frames, only return the first 10 video frames and vice versa
        if (frame_list) != pose_output.shape[0]:
    ...
    def read_pose_files(self, index: int):
        ...
        with open(file_path, 'rb') as f:
            pose_keypoints = pickle.load(f)  # T K(133) 
    ...
    def collate_fn(batch):
        ...
        if len(video[0].shape) > 3:
            ...
            #Pad video
            #breakpoint()
            #video = [element.unsqueeze(0) for element in video[0]]
            video = [frame.unsqueeze(0) for vid in video for frame in vid]
            ...
            #
            #breakpoint()
            #pose_output = [element.unsqueeze(0) for element in pose_output[0]]
            pose_output = [frame.unsqueeze(0) for element in pose_output for frame in element]
            #print("pose data length: ", len(pose_output))
            #print("input pose data raw shape: ", pose_output[0].shape)
            ...
            #breakpoint()
            #pose_weights = [element.unsqueeze(0) for element in pose_weights[0]]
            pose_weights = [frame.unsqueeze(0) for element in pose_weights for frame in element]
            #breakpoint()
