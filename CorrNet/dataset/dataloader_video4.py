...
class BaseFeeder(data.Dataset):
    ...
    def collate_fn(batch):
        ...
        if len(video[0].shape) > 3:
            ...
            padded_video = torch.stack(video)
            ...
            padded_pose = torch.stack(pose_output, dim=0)#.squeeze(1)
            #padded_pose = torch.flatten(padded_pose, start_dim =0 , end_dim = 1)
            ...
            #breakpoint()
            padded_pose_weights = torch.stack(pose_weights, dim=1).squeeze(1).reshape(-1, 133, 1)
