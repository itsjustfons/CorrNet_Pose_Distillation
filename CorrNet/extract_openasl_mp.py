...
def load_corrnet():
    ...
    model = load_model_weights(model, '/data/group1/z40575r/CorrNet_pose_distillation/CorrNet/work_dir/keypoint_regression_img/_best_model.pt')
    #move model to GPU
...
def main():
    ...
    arg_dict = {
        ...
        'output_root': 'keypoints_regression_img3',
        'args': args,
    }
...
def process_single_video(video_path, arg_dict, corrnet_model):
    ...
    #Read video with opencv just to check if it is correct
    ...
    while True:
        ...
        if not ret:
            break
        # Convert BGR → RGB (so it matches MMCV output)
