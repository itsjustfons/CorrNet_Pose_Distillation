...
class TransBaseModel(nn.Module):
    ...
    def add_args(parser):
        ...
        # Backbone config
        parser.add_argument('--pose_backbone', type=str, default='PartedPoseBackbone')
    ...
    def forward(self, x, x_length, tgt=None, tgt_length=None, vn_idxs=None, vn_len=None, phase='train'): #originally phase = train
        ...
        #print("encoder_out_length.shape", encoder_out_length.shape)
        ...
        if phase == 'train':
            ...
            #breakpoint()
            #print("vn_idxs shape:",vn_idxs.shape)
            #check if there is an empty vector in vn_idxs here
            if vn_idxs is None or vn_idxs.shape[0] == 0:
                print("empty vn_idxs detected")
            #breakpoint()
    ...
    def select_glofe_keypoints(self, x):
        """
        x: [B, T, 133, 3]
        returns: pose_output [B, T, 76, 3]
        """
        # indices copied from original dataloader
        ...
        # Hands are contiguous ranges in MMPose format
        hand_right = x[:, :, 91:112, :]   # [B,T,21,3]
        hand_left  = x[:, :, 112:,  :]    # [B,T,21,3]

        body = x[:, :, body_sample_indices, :]       # [B,T,11,3]
        face = x[:, :, face_sample_indices, :]       # [B,T,23,3]

        # concatenate in the same order
        pose = torch.cat([body, hand_left, hand_right, face], dim=2)  # [B,T,76,3]
    ...
    def visual_step(self, x, x_length):
        ...
        x = self.select_glofe_keypoints(x)
        #breakpoint()
        x = x.permute(0,3,1,2)
       ...
       # print("after positional encoding visual_feat.shape", visual_feat.shape)
        '''
        print("print all inputs for make_padding_mask")
        print("visual_feat",visual_feat.shape)
        print("x.shape", x.shape)
        print("x.shape[-2]", x.shape[-2])
        print("visual_feat.shape[0]", visual_feat.shape[0])
        print("visual_feat.shape[1]", visual_feat.shape[1])
        '''
        if self.args.mask_enc:
            ...
            #print("max_x_length", max_x_length, type(max_x_length))
            #print("max_feat_length", max_feat_length, type(max_feat_length))
            #print("ratio", ratio)
            # padding mask for encoder
            length_scaled = torch.ceil(x_length / ratio).type(torch.long)
            #print("length_scaled", length_scaled)
        ...
        #Checking dimensions for the input to the transformer encoder
        #print("encoder inputs")
        #print("visual_feat.shape", visual_feat.shape)
        #print("visual_padding_mask.shape", visual_padding_mask.shape)
